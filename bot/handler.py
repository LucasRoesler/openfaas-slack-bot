...
# To add new slash commands, go back to your app configuration https://api.slack.com/apps/:id
# and add new slackcommands, permissions, etc
@routing.route("/hello_cmd", methods=["GET", "POST"])
async def hello_cmd(request: sanic.request.Request):
    logger.debug("hello")
...
@routing.route("/list-issues", methods=["POST"])
async def list_issues(request: sanic.request.Request):
    data = request.body
    logger.debug(data)
